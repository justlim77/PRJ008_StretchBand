			...
			//if(useHandCursor)
			{
				if((texture != null) && (isLeftHandPrimary || isRightHandPrimary))
				{
					...
					if(controlMouseCursor)
					{
                        ...
                        Vector2 progressPosition = Camera.main.ScreenToViewportPoint(Input.mousePosition);
                        progressPosition.x *= canvasScaler.referenceResolution.x;
                        progressPosition.y *= canvasScaler.referenceResolution.y;
